...
export default {
  ...
  created () {
    ...
    if (index > -1) {
      ...
      Echo.join(`room.${this.currentRoom.id}`) // listen to the shared room
        .here((users) => {
      ...
      Echo.private(`room.${this.$root.user.id}`) // listen to user's own room (in order to receive all private messages from other users)
        .listen('MessagePosted', e => {
